...
public class KIT_NHD_API {

    //update("CREATE TABLE IF NOT EXISTS KIT_NHD (UUID VARCHAR(100), KILLS int, DEATHS int, ELO int, STILL_RANKING VARCHAR(100))");

    public boolean playerExists(UUID uuid) {
        ResultSet rs = MySQL.getResult("SELECT UUID FROM KIT_NHD WHERE UUID='" + uuid + "'");
        try {
            while (rs.next()) {
                if (rs != null) {
                    return true;
    ...
    public void createPlayer(UUID uuid) {
        if (!playerExists(uuid)) {
            MySQL.update("INSERT INTO KIT_NHD (UUID, KILLS, DEATHS, ELO, WINS, RANK_WINS, STILL_RANKING) VALUES ('" + uuid + "', 0, 0, 0, 0, 0, 'true');");
        }
    }
    ...
    public void deleteData() {
        MySQL.update("DELETE FROM KIT_NHD");
    }
