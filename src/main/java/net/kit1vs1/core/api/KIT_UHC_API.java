...
public class KIT_UHC_API {

    public boolean playerExists(UUID uuid) {
        ResultSet rs = MySQL.getResult("SELECT UUID FROM KIT_UHC WHERE UUID='" + uuid + "'");
        try {
            while (rs.next()) {
                if (rs != null) {
                    return true;
    ...
    public void createPlayer(UUID uuid) {
        if (!playerExists(uuid)) {
            MySQL.update("INSERT INTO KIT_UHC (UUID, KILLS, DEATHS, ELO, WINS, RANK_WINS, STILL_RANKING) VALUES ('" + uuid + "', 0, 0, 0, 0, 0, 'true');");
        }
    }
    ...
    public void deleteData() {
        MySQL.update("DELETE FROM KIT_UHC");
    }
