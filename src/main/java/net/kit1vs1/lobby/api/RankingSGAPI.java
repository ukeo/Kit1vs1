...
public class RankingSGAPI {
    ...
    public static void loadRanking() {
        ...
        String n = "NORTH";
        try {
            PreparedStatement ps = MySQL.getConnection().prepareStatement("SELECT UUID FROM KIT_SG ORDER BY RANK_WINS DESC LIMIT 3");
            rs = ps.executeQuery();
            int zahl = 0;
            while (rs.next()) {
                if (!Kit1vs1.getInstance().getKitSgApi().getBoolean(UUID.fromString(rs.getString("UUID")), "KIT_SG", "STILL_RANKING")) {
                    map.put(++zahl, rs.getString("UUID"));
                }
            }
            ArrayList<Location> locs = new ArrayList<Location>();
            locs.add(LobbyLocationAPI.getLocation("SgHead1"));
            locs.add(LobbyLocationAPI.getLocation("SgHead2"));
            locs.add(LobbyLocationAPI.getLocation("SgHead3"));
            int i = 0;
            while (i < locs.size()) {
                int id = i + 1;
                ...
                if (newloc.getBlock().getState() instanceof Sign) {
                    ...
                    Sign sign = (Sign)b;
                    if (legit) {
                        double deaths = Kit1vs1.getInstance().getKitSgApi().getInt(UUID.fromString(String.valueOf(map.get(id))), "KIT_SG", "DEATHS");
                        double kills = Kit1vs1.getInstance().getKitSgApi().getInt(UUID.fromString(String.valueOf(map.get(id))), "KIT_SG", "KILLS");
                        double kd = 0;
                        if (deaths == 0) {
                            kd = kills + 0.00;
                        ...
                        sign.setLine(0, "- SG Platz #" + id + " -");
                        sign.setLine(1, "\u00a70" + name);
                        sign.setLine(2, Kit1vs1.getInstance().getKitSgApi().getInt(UUID.fromString(String.valueOf(map.get(id))), "KIT_SG", "RANK_WINS") + " Gold-Wins");
